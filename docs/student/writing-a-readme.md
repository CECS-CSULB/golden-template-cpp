...
- How can someone build, run, and test it?
...
A C++ command-line program that reads assignment scores and reports summary
...
This project began as an assignment in an introductory C++ course. I
implemented the statistics module, input validation, and automated tests. The
starter repository supplied the CMake configuration and the initial program
interface.
~~~
...
- Uses CMake and GoogleTest for repeatable builds and tests.
...
~~~markdown
**Technology:** C++, CMake, vcpkg, GoogleTest, GitHub Actions
~~~
...
```text
$ ./build/bin/statistics scores.txt
Count: 5
Average: 84.60
Minimum: 72
Maximum: 96
...
compiler or runtime versions, package managers, and other dependencies. Then
...
with a relative path, for example `[Build guide](docs/student/setup.md)`.
...
```bash
./build/bin/statistics path/to/scores.txt
```
...
- Update the README when the build commands, dependencies, or behavior change.
...
- [ ] Requirements, build, usage, and test commands work from a clean clone.
